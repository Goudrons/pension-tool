...
app.secret_key = "4c3559003b6f65a195e4b4290e2d3a64ddd3dc3df1082349fcd618a344f54e39"
...
STYLE = """
<style>
:root{--bg:#0b1120;--panel:#111827;--border:#1f2937;--text:#e5e7eb;--muted:#94a3b8;--blue:#2563eb;--green:#22c55e;--red:#ef4444}
*{box-sizing:border-box}
body{margin:0;font-family:Arial,sans-serif;background:radial-gradient(circle at top left,#1e3a8a33,transparent 35%),var(--bg);color:var(--text)}
...
.card{background:linear-gradient(180deg,#111827,#0f172a);border:1px solid var(--border);border-radius:18px;padding:18px;box-shadow:0 16px 40px #0005}
...
.field{background:#02061766;border:1px solid #1e293b;border-radius:14px;padding:13px}
label{display:block;font-size:12px;color:#cbd5e1;margin-bottom:7px;line-height:1.35}
input,select,textarea{width:100%;padding:11px;border-radius:10px;border:1px solid #334155;background:#020617;color:var(--text);font-size:15px}
button,.btn{display:inline-block;padding:12px 16px;border:0;border-radius:12px;background:var(--blue);color:white;font-size:15px;cursor:pointer;text-decoration:none;font-weight:bold}
.btn.gray,button.gray{background:#374151}.btn.red{background:#991b1b}
.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px;position:sticky;bottom:16px;background:#0f172acc;backdrop-filter:blur(8px);border:1px solid #1f2937;padding:12px;border-radius:16px}
.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px}
.kpi{background:linear-gradient(180deg,#111827,#020617);border:1px solid #243244;border-radius:18px;padding:18px}
.kpi.main{grid-column:span 1;padding:24px;border-color:#334155}
.kpi.sub{grid-column:span 2}
.kpi-label{color:var(--muted);font-size:13px;margin-bottom:8px}.kpi-value{font-size:28px;font-weight:bold;letter-spacing:-.03em;animation:fadeInUp .4s ease}
...
table{width:100%;border-collapse:collapse;font-size:14px}th,td{text-align:left;padding:12px;border-bottom:1px solid #1f2937}th{color:#cbd5e1;font-size:12px;background:#02061799}.num{text-align:right;white-space:nowrap}
img{width:100%;border-radius:14px;background:white;margin-top:10px}
.scenario-item{background:#02061766;border:1px solid #1e293b;border-radius:14px;padding:13px;margin-bottom:10px}
.scenario-name{font-weight:bold;margin-bottom:8px}.scenario-meta{color:var(--muted);font-size:12px;line-height:1.5;margin-bottom:10px}
...
body{
  background:
    radial-gradient(circle at top left,#2563eb26,transparent 32%),
    radial-gradient(circle at top right,#14b8a61c,transparent 28%),
    linear-gradient(180deg,#0b1120,#020617 65%);
}
...
.card{
  background:
    linear-gradient(180deg,#111827f2,#0f172af2),
    radial-gradient(circle at top left,#3b82f61a,transparent 35%);
  border-color:#273449;
}
...
.kpi:before{
  ...
  position:absolute;
  inset:0;
  background:linear-gradient(135deg,#60a5fa18,transparent 45%);
  pointer-events:none;
}

.kpi.main{
  border-color:#3b82f6;
  box-shadow:0 18px 50px #1d4ed833;
}
...
.scenario-item.active{
  ...
  box-shadow:0 0 0 1px #60a5fa55,0 14px 32px #1d4ed822;
}
...
.tooltip-icon:hover{border-color:#60a5fa;color:#60a5fa}
.tooltip-box{display:none;position:absolute;top:calc(100% + 6px);left:50%;transform:translateX(-50%);background:#0f172a;border:1px solid #334155;border-radius:10px;padding:8px 12px;font-size:12px;color:#cbd5e1;white-space:nowrap;z-index:9999;box-shadow:0 8px 24px rgba(0,0,0,.4);pointer-events:none;line-height:1.5;min-width:200px;max-width:300px;white-space:normal}
.tooltip-box:before{content:"";position:absolute;bottom:100%;left:50%;transform:translateX(-50%);border:6px solid transparent;border-bottom-color:#334155}
...
.compare-neutral{color:#cbd5e1;font-weight:700}
...
body{
  background:
    radial-gradient(circle at top right,#1e293b 0%,#0f172a 42%,#020617 100%)!important;
}
...
<div id="tour-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:99999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px)">
  <div style="background:#0f172a;border:1px solid #334155;border-radius:24px;padding:32px;max-width:480px;width:90%;box-shadow:0 32px 80px rgba(0,0,0,.6)">
    ...
    <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:24px">
      <div style="display:flex;gap:12px;align-items:flex-start;background:#1e293b;border-radius:12px;padding:12px">
...
<div style="text-align:center;padding:14px;margin-bottom:18px;background:rgba(96,165,250,.08);border:1px solid rgba(96,165,250,.15);border-radius:14px;font-size:15px;color:#94a3b8">
  {{ result.fun_message }} <span style="color:#64748b;font-size:12px">· Noch {{ result.days_to_pension }} Tage bis zur Freiheit!</span>
</div>
{% endif %}
...
def init_db():
    ...
    try:
        ...
        pass
    # Fehlende Spalten in scenarios hinzufügen
    missing_cols = {
    ...
    <div class="card" style="padding:28px">
      ...
      <form method="post">
        <div class="field" style="margin-bottom:14px">
          ...
          <div id="pw-strength" style="margin-top:6px;height:4px;border-radius:99px;background:#1e293b;overflow:hidden"><div id="pw-bar" style="height:100%;width:0%;transition:.3s;border-radius:99px"></div></div>
        ...
        <div style="background:#1e293b;border-radius:10px;padding:12px;margin-bottom:16px;font-size:12px">
...
.hero div{color:#cbd5e1;font-size:14px}
...
def paerchen():
    ...
    if request.method == "POST":
        if id1 and id2:
            ...
            pass  # Desktop: kein Session-Reset nötig
        if scenario_id:
            conn = db()
